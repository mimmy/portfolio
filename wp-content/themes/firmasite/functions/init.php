...
if ( !defined('"firmasite"') )
	define('"firmasite"', "firmasite"); 
...
function firmasite_enqueue_script() {
	...
	if (isset($firmasite_settings["font"]) && !empty($firmasite_settings["font"])){
		...
		wp_enqueue_style( 'google-webfonts', $firmasite_settings["protocol"] . '://fonts.googleapis.com/css?family=' . $firmasite_settings["font"] . '&amp;subset=latin'. $firmasite_data_subsets );
		add_action ("wp_head", "firmasite_customcss_googlefont");
		function firmasite_customcss_googlefont() {
			global $firmasite_settings;
			?>
			<style type="text/css" media="screen">
				body, h1, h2, h3, h4, h5, h6 { font-family: <?php echo $firmasite_settings["font"]; ?>,sans-serif !important;}
			</style>
			<?php
		}
	}
	...
	if(!isset($firmasite_settings["hover-nav"]) || $firmasite_settings["hover-nav"] != true )
		add_action("wp_footer", "firmasite_hover_nav",999);	
	...
	// jQuery Masonry for Vertical List (Excerpt) aka loop_tile
	if (isset($firmasite_settings["loop-style"]) && "loop-tile" == $firmasite_settings["loop-style"]){
		
		wp_enqueue_script( 'jquery-masonry' );
		
		add_action ("wp_footer", "firmasite_masonry_implement",999);
		function firmasite_masonry_implement() {
			?>
			<script type="text/javascript">
				(function ($) {
					var $loop_tile = $('ul.loop_tile:first');
					$(window).load(throttle(function(){
						$loop_tile.masonry({ itemSelector: '.loop_tile_item', isAnimated: true });
					},250)).trigger("resize");
	...
	// if ($("#visible-lg").css("display") === "block")
	add_action ("wp_footer", "firmasite_bootstrap_check_blocks",999);
...
function firmasite_hover_nav_combine(){ 
	...
	if(!isset($firmasite_settings["hover-nav"]) || $firmasite_settings["hover-nav"] != true ) {?>
		function firmasite_hover_nav() {
			...
			if ($("#visible-md").css("display") === "block" || $("#visible-lg").css("display") === "block" ){
				...
				$("style#hover-nav").remove();
				$('a.dropdown-toggle').each(function(){
					var data_toggle_removed = $(this).attr('data-toggle-removed');
					if (data_toggle_removed && data_toggle_removed.length) $(this).attr('data-toggle',data_toggle_removed).removeAttr('data-toggle-removed');
				});						
			}
		}
		$(window).resize(throttle(function(){
